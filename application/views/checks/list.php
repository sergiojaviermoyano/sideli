      ...
      'ajax':{
          ...
          "dataSrc": function (json) {
            ...
            $.each(json.data,function(index,item){              
              ...
              var td_3=item.rsbco;
              var td_4= (item.rsag != null && item.rsag != '' ?  item.rsag + ' / ' : '') + item.apellido + ' ,' + item.nombre;
              var td_5='$ '+ item.importe;
              var d = new Date(item.vencimiento);
              var td_6= d.getDate()  + "-" + (d.getMonth()+1) + "-" + d.getFullYear();
              switch(item.estado){
                case 'AC':
                  var td_7='<center><small class="label bg-green">AC</small></center>';
                  break;

                case 'IN':
                  var td_7='<center><small class="label bg-yellow">IN</small></center>';
                  break;
              }
